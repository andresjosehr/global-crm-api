...
🤓 Este es el estado actual de tus cursos:
@foreach ($coursesToNotify as $course)
...
Todavía HOY puedes extender por DOS MESES ({{implode(', ', $pendingCoursesToNotifyNames) }}), pero en pocos días, la extensión mínima es de 3 meses *sin excepción.*

@if( count($noFreeAttemptsCoursesToNotifyNames) == 1)
🚨 Además, puedes pagar para *PONDERAR* los resultados de tus exámenes + el avance académico completado en tu aula virtual y obtener el certificado de ({{implode(', ', $noFreeAttemptsCoursesToNotifyNames) }}) en un máximo de 48 horas hábiles.
@elseif ( count($noFreeAttemptsCoursesToNotifyNames) > 1)
🚨 Además, puedes pagar para *PONDERAR* los resultados de tus exámenes + el avance académico completado en tu aula virtual y obtener los certificados de ({{implode(', ', $noFreeAttemptsCoursesToNotifyNames) }}) en un máximo de 48 horas hábiles.
@endif
