...
class FreeCourseExtensions extends Controller
{
    public function index()
    {
        ...
        $studentsFiltered = array_map(function ($student) {
            $freeCourses = array_filter($student['courses'], function ($course) {
                if (is_null($course['end']) || is_null($course['start']) || $course['type'] == 'paid' || !isset($course['certifaction_test'])) {
                    return false;
                }
                ...
                if ($course['course_id'] == 6) {
                    ...
                    foreach ($niveles as $nivel) {
                        if ($course[$nivel]['certifaction_test'] != 'INTENTOS PENDIENTES') {
                            return false;
                        }
                    }
                }
                ...
                if ($course['course_id'] != 6) {
                    if ($course['certifaction_test'] != 'INTENTOS PENDIENTES') {
                        return false;
                    }
                }
            ...
            $student['extension_free_courses_msg'] = $freeCourses;
            return $student;
        }, $students);

        $studentsFiltered = array_values($studentsFiltered);
        $studentsFiltered = array_filter($studentsFiltered, function ($student) {
            return isset($student['extension_free_courses_msg']) && count($student['extension_free_courses_msg']) > 0;
        });
        $studentsFiltered = array_values($studentsFiltered);
        ...
        foreach ($studentsFiltered as $i => $student) {
            foreach ($student['extension_free_courses_msg'] as $j => $msg) {
                // Get blade template
                $text = view('especial-messages.free-courses-extension.' . $msg['template'], $msg)->render();
                // remove \n consecutives. Only two \n
                $text = preg_replace("/\n\n+/", "\n\n", $text);

                $studentsFiltered[$i]['extension_free_courses_msg'][$j]['text'] = $text;
            }
        }
        ...
        foreach ($sheets as $sheet) {
            if (isset($groupedStudents[$sheet])) {
                ...
                foreach ($groupedStudents[$sheet] as $student) {
                    ...
                    $message   = $student['extension_free_courses_msg'][0]['text'];  // Obtener el mensaje a enviar
